...
const Navbar = () => {
    ...
    const fetchNotificationCounts = async () => {
      try {
        ...
        const unreadResponse = await axios.get(
          BASE_URL + "/chat/unread-counts",
          { withCredentials: true },
        );
        const counts = unreadResponse.data?.counts ?? {};
        if (Object.keys(counts).length > 0) {
          dispatch(hydrateUnreadCounts(counts));
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    socket.on("receiveMessage", ({ userId: senderId, firstName }) => {
      if (!senderId || String(senderId) === String(user._id)) return;
      const activeChatPath = `/chat/${senderId}`;
      if (location.pathname === activeChatPath) return;
      dispatch(incrementUnreadCount(senderId));
      setToast({
        type: "success",
        message: `New message from ${firstName || "a developer"}`,
      });
    });
  ...
  }, [user?._id, dispatch, location.pathname]);
                ...
                >
                  ...
                  <li>
                    <a
                      href="#features"
                      onClick={(event) =>
                        handleLandingSection(event, "features")
                      }
                    >
                      Features
                    </a>
